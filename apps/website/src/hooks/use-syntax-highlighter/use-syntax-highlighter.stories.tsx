...
import { useSyntaxHighlighter } from '.';

type Props = { language: 'css' | 'shell' | 'tsx' | 'typescript'; lines: string[]; words: string[] };
type Story = StoryObj<Props>;

const meta: Meta<Props> = {
  title: 'HOOKS/useSyntaxHighlighter',
  args: {
    language: 'css',
    lines: undefined,
    words: undefined,
...
export const Example: Story = {
  render: (props) => {
    const { language, lines, words } = props;

    const Example = component$(() => {
      const highlightedHTML = useSyntaxHighlighter(code[language], language, { lines, words });

      return <div dangerouslySetInnerHTML={highlightedHTML} />;
    });
